const addresses = require("../addresses/mooney") // Get all relevant Ethereum and BSC addresses
const Abi = require("../abi/abi.json") // Get the token ABI for the project. ABIs can be found on the Etherscan page for the contract if the contract has been verified. Otherwise you may need to ask your Solidity dev for it.
...
const getMooneyData = async (web3s) => {
    ...
    let mooney = new web3.eth.Contract(Abi, eth_addresses.contract)
    // Here we instantiate the BSC smart contract object
    let bsc_projectOne
    try {
        bsc_projectOne = new bsc_web3.eth.Contract(Abi, bsc_addresses.contract)
    } catch (err) {
    ...
    const burnt_on_eth = await mooney.methods.balanceOf(eth_addresses.burnt).call()

    tokenData.eth.totalSupply.value = await mooney.methods.totalSupply().call()
    const team_1 = await mooney.methods.balanceOf(eth_addresses.team_1).call()
    const team_2 = await mooney.methods.balanceOf(eth_addresses.team_2).call()
    const team_3 = await mooney.methods.balanceOf(eth_addresses.team_3).call()
    ...
    tokenData.eth.circulatingSupply.value = Number(tokenData.eth.totalSupply.value) - Number(burnt_on_eth) - Number(team_eth)
    ...
    // Set up descriptions 
    tokenData.eth.totalSupply.description = "Total supply of mooney on ETH"
    tokenData.bsc.totalSupply.description = "Total supply of mooney on BSC"

    tokenData.eth.circulatingSupply.description = "Circulating supply of mooney on ETH"
    tokenData.bsc.circulatingSupply.description = "Circulating supply of mooney on BSC"

    tokenData.combined.totalSupply.description = "Total supply of mooney (BSC & ETH)"
    tokenData.combined.circulatingSupply.description = "Circulating supply of mooney (BSC & ETH)"

    // Set names

    tokenData.eth.totalSupply.name = "Total Supply of mooney on ETH"
    tokenData.bsc.totalSupply.name = "Total Supply of mooney on BSC"

    tokenData.eth.circulatingSupply.name = "Circulating Supply of mooney on ETH"
    tokenData.bsc.circulatingSupply.name = "Circulating Supply of mooney on BSC"

    tokenData.combined.totalSupply.name = "Total Supply of mooney on (BSC & ETH)"
    tokenData.combined.circulatingSupply.name = "Circulating Supply of mooney on (BSC & ETH)"
    ...
    try {
        ...
        db.updateMooneyData(tokenData, client)
    } catch (err) {
        console.log(err)
    }
}

module.exports = getMooneyData